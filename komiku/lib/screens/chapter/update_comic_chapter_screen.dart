...
class _UpdateComicChapterScreenState extends State<UpdateComicChapterScreen> {
  ...
  final List<File> _newPages = [];
  final List<Uint8List> _newPagesBytesWeb = [];
      ...
      if (response['status'] == 'SUCCESS') {
        ...
        setState(() {
          if (data is List && data.isNotEmpty) {
            ...
            _numberController.text = (data.first['chapter_number'] ?? '')
                .toString();
  ...
  Future<void> _submit() async {
    ...
    try {
      final chapterResponse = await ApiService.updateComicChapter(
        Chapter(
          id: widget.chapterId,
      ...
      if (chapterResponse['status'] != 'SUCCESS') {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                chapterResponse['error_messages']?.toString() ??
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    image: DecorationImage(
                                      image:
                                          kIsWeb &&
                                              index < _newPagesBytesWeb.length
                                          ? MemoryImage(
                                              _newPagesBytesWeb[index],
                                            )
                                          : FileImage(_newPages[index])
                                                as ImageProvider,
                                      fit: BoxFit.cover,
                                    ),
                                  ),
